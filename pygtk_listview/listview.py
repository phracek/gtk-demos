...
GLADE_FILE = os.path.join(os.path.dirname(__file__), 'listview.glade')
...
class ListViewExample(object):
    def __init__(self):
        ...
        column = Gtk.TreeViewColumn("Available versions", renderer, markup=0)
    ...
    def btn_browse_click(self, widget):
        ...
        if not os.path.exists(git_config):
            ...
            for version in sorted(versions):
                self.combo_box.append_text(version)
    ...
    def list_view_cursor_changed(self, listview):
        ...
        if listiter is not None:
            print ('changed row: %s' % model[listiter][0])
